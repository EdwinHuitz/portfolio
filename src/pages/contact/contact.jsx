...
export default function Contact(){
   ...
   function validateForm(e){
      ...
      if(formI.name && formI.email && formI.subject && formI.message){
         ...
         axios.post('https://6xiseb1eui.execute-api.us-east-1.amazonaws.com/dev/services'+[formI.name,formI.email,formI.subject,formI.message])
         .then(e=>setMsg(2))
         .catch(e=>setMsg(3))
      }
      e.preventDefault()
               ...
               <form id ="contact_form" onSubmit={validateForm} className="row email-form">
                  <div id="msg_sender_info" className="row">
                     <div id="msg_name" className="col">
                        <input type="text" name="name" title="Name" className={formI.name.length>=2?"form-control is-valid":"form-control is-invalid"} id="name" placeholder='Your Name' onChange={(e)=>updateValues(e)} required />
                     ...
                     <div id="msg_email" className="col">
                        <input type="text" className={!eMail?"form-control is-invalid":"form-control is-valid"} name="email" title="E-Mail Address" id="email" value={formI.email} placeholder="Your E-Mail Address" onChange={(e)=>{updateValues(e);validateEmail()}} required />
                  ...
                  <div id="msg_subject" className="row">
                     <input type="text" className={formI.subject.length>=5?"form-control is-valid":"form-control is-invalid"} name="subject" title="Subject" id="subject" value={formI.subject} placeholder="Subject" onChange={(e)=>updateValues(e)} required />
                  ...
                  <div id="msg_text" className="row">
                     <textarea className={formI.message.length>=10?"form-control is-valid":"form-control is-invalid"} name="message" title="Message" id="message" rows="10" value={formI.message} placeholder="Message" onChange={(e)=>updateValues(e)} required ></textarea>
